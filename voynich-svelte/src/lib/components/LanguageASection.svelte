<script>
	import { COMPARISON, FOLIOS } from '$lib/language-a-data.js';
	import { STATS } from '$lib';
</script>

<div class="sprache-a">
	<div class="box hl">
		<div class="box-title">Sprache A vs. B — Hauptunterschiede</div>
		<div class="table-wrap">
			<table class="dt">
				<thead>
					<tr><th>Merkmal</th><th>Sprache B (f57r, f103r)</th><th>Sprache A ({STATS.foliosA})</th></tr>
				</thead>
				<tbody>
					{#each COMPARISON as c}
						<tr>
							<td class="feature-cell">{c.feature}</td>
							<td class="note-cell">{c.langB}</td>
							<td class="note-cell">{c.langA}</td>
						</tr>
					{/each}
				</tbody>
			</table>
		</div>
	</div>

	<h3>Quires A–C — Textkorpus (alle analysierten Folios)</h3>
	<div class="folio-wrap">
		<table class="dt">
			<thead>
				<tr><th>Folio</th><th>Pflanze</th><th>Textsignal</th><th>Konf.</th></tr>
			</thead>
			<tbody>
				{#each FOLIOS as f}
					<tr>
						<td class="folio-cell">{f.folio}</td>
						<td>{f.plant}</td>
						<td class="note-cell">{f.signal}</td>
						<td><span class={f.is5 ? 'conf5' : 'conf'}>{f.stars}</span></td>
					</tr>
				{/each}
			</tbody>
		</table>
	</div>

	<div class="box green conclusion">
		<div class="box-title">Schlussfolgerung: Sprache A = Sprache B, dialektale Variante — Quire C als Übergangszone</div>
		<p style="margin:0;font-size:.92rem">Die Sprachen A und B sind dialektale Varianten desselben Mischna-Hebräisch/Aramäisch-Systems: A hat höhere o-Vokalfrequenz (Ḥolam), B mehr e-Vokale (Segol/Ṣere). Textgenre, Prognose-Schema, Präfix-System und Kolophon-Konventionen sind identisch. Die Quire-Grenze A→B (f8v→f9r) ist textlich unsichtbar. <strong>Quire C (f17–f18+)</strong> zeigt erstmals B-Merkmale in Sprache-A-Texten: das <span class="eva">qo-</span>-Präfix tritt auf (13 Belege f17r/f17v + weitere in f18r/f18v, R33 ★★★); <span class="eva">sheol</span> (Vollform) erscheint am Paragraphenende (R34 ★★★); <span class="eva">qokchy·qokchy</span> und <span class="eva">qoky·qoky</span> belegen qo-Präfix-Dopplung (R37 ★★★). Neu in f18v: erste zoomorphe Begleitfigur im botanischen Corpus (Drachen-Figur, Signaturenlehre) und <span class="eva">dom=</span> als Kolophon-Finale in Sprache A (R39 ★★★) — Quire C ist eine dialektale und ikonographische Übergangszone. Das Werk ist als Einheit konzipiert.</p>
	</div>
</div>

<style>
	.feature-cell {
		color: var(--ink-f);
		font-size: .85rem;
		white-space: nowrap;
	}

	.note-cell {
		color: var(--ink-f);
		font-size: .82rem;
	}

	.folio-cell {
		font-family: var(--font-mono);
		color: var(--ink-f);
		font-size: .82rem;
		white-space: nowrap;
	}

	.folio-wrap {
		overflow-x: auto;
	}

	.conclusion {
		margin-top: 1.2rem;
	}
</style>
